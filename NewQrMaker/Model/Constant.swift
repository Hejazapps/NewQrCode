...
extension Constant {
    ...
    static func createWifi(modelArray: [ResultDataModel]) -> String {
        ...
        print("✅ WiFi MA: \(modelArray)")

        for data in modelArray {
            switch data.title {
            case "Network Name:":
                ssid = data.description
            case "Password:":
                password = data.description
            case "Encription:":
                let desc = data.description.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
                switch desc {
                case "NONE":
                    encription = "nopass"
                case "", "WAP", "WAP/WAP2":
            ...
            case "Hidden:":
                hidden = data.description.lowercased() == "true" ? "true" : "false"
            default:
                print("⚠️ Unknown title: \(data.title)")
            }
        }
        ...
        var wifi = "WIFI:T:\(encription);S:\(ssid);"

        if encription != "nopass" {
            if password.count >= 8 {
                wifi += "P:\(password);"
            } else {
