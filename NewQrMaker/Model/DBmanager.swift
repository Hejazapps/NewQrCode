...
class DBmanager: NSObject {
    ...
    private func manageDBLocation() {
        do {
            ...
            let dbURL = dbDirectory.appendingPathComponent("qrCode.sqlite")
            let dbPath = dbURL.path

            // Remove any existing item at destination path
            if fileManager.fileExists(atPath: dbPath) {
                do {
                    try fileManager.removeItem(atPath: dbPath)
                } catch {
                    print("Error removing existing DB file: \(error)")
                    return
                }
            }

            // Locate bundled DB
            guard let dbResourcePath = Bundle.main.path(forResource: "qrCode", ofType: "sqlite") else {
            ...
            print("Error managing DB location: \(error)")
        }
    }
    ...
    func getRecordInfo(indexPath: String, completion: @escaping ([DataInformation]) -> Void) {
        queue.async { [weak self] in
            ...
            let stmt =  "SELECT id,Text,indexPath,codeType,position,shape,logo,folderid,temp,tempText,fontcolor,fontfamily,fontsize,pupil FROM record  where indexPath = ('\(indexPath)') order by id DESC"
            if (sqlite3_open(DBpath, &db)==SQLITE_OK)
            {
                ...
                if sqlite3_prepare_v2(db, stmt, -1, &queryStatement, nil) == SQLITE_OK {
                    ...
                    while (sqlite3_step(queryStatement) == SQLITE_ROW){
                        ...
                        let temp =  sqlite3_column_text(queryStatement, 8)
                        let tempText =  sqlite3_column_text(queryStatement, 9)
                        let fontcolor =  sqlite3_column_text(queryStatement, 10)
                        let fontfamily =  sqlite3_column_text(queryStatement, 11)
                        let fontsize =  sqlite3_column_text(queryStatement, 12)
                        let pupil =  sqlite3_column_text(queryStatement, 13)
                        ...
                        let str8 = String(cString: temp!)
                        let str9 = String(cString: tempText!)
                        let str10 = String(cString: fontcolor!)
                        let str11 = String(cString: fontfamily!)
                        let str12 = String(cString: fontsize!)
                        let str13 = String(cString: pupil!)
    ...
    func getFolderElements(folderid: String, completion: @escaping ([DataInformation]) -> Void) {
       queue.async { [weak self] in
           ...
           let stmt =  "SELECT id,Text,indexPath,codeType,position,shape,logo,folderid,temp,tempText,fontcolor,fontfamily,fontsize,pupil FROM record  where folderid = ('\(folderid)') order by id DESC"
           if (sqlite3_open(DBpath, &db)==SQLITE_OK)
           {
    ...
    func updateTableData(id: String,Text: String,position: String,shape:String,logo:String,temp:String,tempText:String,fontcolor:String,fontfamily:String,fontsize:String,pupil:String)
    {
        print("actual temp i am getting \(temp)")
        queue.async { [weak self] in
            guard let self else {
                print("Can't make self strong!")
                return
            ...
            let createTable = "UPDATE record SET Text ='\(Text)',position ='\(position)',shape ='\(shape)',logo ='\(logo)',temp ='\(temp)',tempText ='\(tempText)',fontcolor ='\(fontcolor)',fontfamily ='\(fontfamily)',fontsize ='\(fontsize)',pupil ='\(pupil)' WHERE id ='\(id)'"
            if (sqlite3_open(DBpath, &db)==SQLITE_OK)
            {
    ...
    func insertRecordIntoFile(Text: String,codeType:String,indexPath:String,position: String,shape:String,logo:String,temp:String,tempText:String,fontcolor:String,fontfamily:String,fontsize:String,pupil:String) {
        queue.async { [weak self] in
            guard let self else {
                print("Can't make self strong!")
                return
            ...
            let insertData = "INSERT INTO record (Text,codeType,indexPath,position,shape,logo,temp,tempText,fontcolor,fontfamily,fontsize,pupil) VALUES  ('\(Text)','\(codeType)','\(indexPath)','\(position)','\(shape)','\(logo)','\(temp)','\(tempText)','\(fontcolor)','\(fontfamily)','\(fontsize)','\(pupil)')"
            let  rc = sqlite3_open_v2(DBpath, &db, SQLITE_OPEN_READWRITE , nil);
